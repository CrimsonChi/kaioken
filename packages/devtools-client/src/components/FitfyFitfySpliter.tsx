...
export const FiftyFiftySplitter: Kaioken.FC = (props) => {
  ...
  const onMouseMove = useCallback<NonNullable<ElementProps<'div'>['onmousemove']>>((e) => {
    ...
    offsetDistance.value.y = prevOffsetDistance.value.y + startMouse.value.y - e.y
    offsetDistance.notify()
  }, [])

  useEventListener('mousemove', onMouseMove)

  return <main ref={mainContainer} className="flex-grow grid gap-2 items-start w-full relative" style={{ gridTemplateColumns: `${firstContainerWidth}px 1fr` }}>
  <div ref={firstViewContainer} className="firstContainer w-full h-full">
    ...
    />
    <div className="secondContainer h-full">
      {secondView}
    </div>
  </main>
}
